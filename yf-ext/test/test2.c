...
int yf_test_2(void) {
  ...
  assert(dtb);
  const unsigned dtb_allocs = 3;
  if (yf_dtable_alloc(dtb, dtb_allocs) != 0)
    assert(0);
  ...
  YF_vinput vin = {attrs, 2, sizeof(struct L_vertex), YF_VRATE_VERT};
  ...
  // BMP image data
  YF_texdt tdt;
  if (yf_filetype_bmp_load(getenv("BMPTEST"), &tdt) != 0)
    assert(0);
  size_t tsz = tdt.dim.width * tdt.dim.height;
  if (tdt.pixfmt == YF_PIXFMT_RGB8SRGB)
    tsz *= 3;
  else if (tdt.pixfmt == YF_PIXFMT_RGBA8SRGB)
    tsz *= 4;
  else
    assert(0);
  const void *pixs[3] = {tdt.data, tdt.data, tdt.data};

  // Image (shader res.)
  const YF_dim3 shd_img_dim = {tdt.dim.width, tdt.dim.height, 1};
  YF_image shd_img = yf_image_init(ctx, tdt.pixfmt, shd_img_dim, 3, 1, 1);
  assert(shd_img != NULL);
  ...
  // Data copy
  YF_slice layers = {0, 1};
  for (unsigned i = 0; i < dtb_allocs; ++i) {
    if (yf_image_copy(shd_img, layers, pixs[i], tsz) != 0)
      assert(0);
    ++layers.i;
  }
  ...
  assert(cb != NULL);
  YF_image tmp_img = yf_image_init(ctx, tdt.pixfmt, shd_img_dim, 3, 1, 1);
  assert(tmp_img != NULL);
  yf_cmdbuf_copyimg(cb, tmp_img, 0, shd_img, 0, 2);
  ...
  size_t offs;
  size_t sz = sizeof l_mats[0];
  YF_slice lay_slc = {0, 1};
  for (unsigned i = 0; i < dtb_allocs; ++i) {
    offs = i * sz;
    if (yf_dtable_copybuf(dtb, i, 0, elems, &buf, &offs, &sz) != 0)
      assert(0);
    if (yf_dtable_copyimg(dtb, i, 1, elems, &shd_img, &lay_slc.i) != 0)
      assert(0);
    ++lay_slc.i;
  }
