...
#define YF_MOVEFAC 0.165
#define YF_TURNFAC 0.045
...
static void init(void) {
  ...
  YF_window win = yf_window_init(ctx, win_dim, "Test 7");
  assert(win != NULL);
  ...
  // Stages
  YF_modid vmod, fmod;
  if (yf_loadmod(ctx, getenv("VMOD_T7"), &vmod) != 0)
    assert(0);
  if (yf_loadmod(ctx, getenv("FMOD_T7"), &fmod) != 0)
    assert(0);
  YF_stage stgs[2] = {
  ...
  YF_vattr attrs[3] = {
    ...
    {1, YF_TYPEFMT_FLOAT2, offsetof(YF_vquad, tc)},
    {4, YF_TYPEFMT_FLOAT4, offsetof(YF_vquad, col)}
  };
  YF_vinput vin = {attrs, 3, sizeof(YF_vquad), YF_VRATE_VERT};
  ...
  // Quad object
  YF_quad quad = yf_quad_init();
  assert(quad != NULL);

  // Quad texture
  YF_texture tex = yf_texture_init(YF_FILETYPE_BMP, getenv("QUAD"));
  assert(tex != NULL);
  yf_quad_settex(quad, tex);
  if (yf_texture_copyres(yf_quad_gettex(quad), dtb, 0, 2, 0) != 0)
    assert(0);
...
static void update(void) {
  ...
  yf_mesh_draw(yf_quad_getmesh(l_vars.quad), cb, 1, 0);
