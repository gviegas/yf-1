...
#define YF_WINWID 800
#define YF_WINHEI 600

#define YF_PARTN 1000
...
static void init(void) {
  ...
  YF_window win = yf_window_init(ctx, win_dim, "Test 6");
  assert(win != NULL);
  ...
  // Stages
  YF_modid vmod, fmod;
  if (yf_loadmod(ctx, getenv("VMOD_T6"), &vmod) != 0)
    assert(0);
  if (yf_loadmod(ctx, getenv("FMOD_T6"), &fmod) != 0)
    assert(0);
  YF_stage stgs[2] = {
  ...
  YF_vattr attrs[3] = {
    ...
    {4, YF_TYPEFMT_FLOAT4, offsetof(YF_vpart, col)}
  };
  YF_vinput vin = {attrs, 2, sizeof(YF_vpart), YF_VRATE_VERT};
  ...
  YF_vec3 cam_orig = {0.0, -30.0, -50.0};
  ...
  // Particle object
  YF_particle part = yf_particle_init(YF_PARTN);
  assert(part != NULL);

  // Sprite texture
  YF_texture tex = yf_texture_init(YF_FILETYPE_BMP, getenv("SPRITE"));
  assert(tex != NULL);
  yf_particle_settex(part, tex);
  if (yf_texture_copyres(yf_particle_gettex(part), dtb, 0, 6, 0) != 0)
    assert(0);
...
static void update(void) {
  ...
  // Particle generation
  static double part_t0 = -1.0;
  static double part_t1 = 0.0;
  if (part_t0 < 0.0) {
    YF_psys *sys = yf_particle_getsys(l_vars.part);
    sys->emitter.size = 0.5f;
    sys->lifetime.duration_min = 0.5f;
    sys->lifetime.duration_max = 2.0f;
    sys->lifetime.spawn_min= sys->lifetime.spawn_max = 0.3f;
    sys->lifetime.death_min = sys->lifetime.death_max = 0.6f;
    //sys->lifetime.once = 1;
    sys->velocity.min[0] = -0.01f;
    sys->velocity.min[2] = -0.01f;
    sys->velocity.max[0] = 0.01f;
    sys->velocity.max[2] = 0.01f;
    sys->color.min[1] = sys->color.max[1] = 0.0f;
    yf_particle_simulate(l_vars.part, 0.0);
    part_t0 = yf_gettime();
    part_t1 = yf_gettime();
  } else {
    part_t1 = yf_gettime();
    yf_particle_simulate(l_vars.part, part_t1 - part_t0);
    part_t0 = part_t1;
  }
  ...
  yf_mesh_draw(yf_particle_getmesh(l_vars.part), cb, 1, 0);
