...
# endif /* defined(VK_USE_PLATFORM_XCB_KHR) */
    ...
    res = vkGetPhysicalDeviceSurfaceSupportKHR(wsi->ctx->phy_dev,
                                               wsi->ctx->pres_queue_i,
                                               wsi->surface, &supported);
    if (res != VK_SUCCESS) {
        yf_seterr(YF_ERR_DEVGEN, __func__);
        return -1;
    }
...
static int create_swapchain(YF_wsi wsi)
{
    ...
    if (wsi->swapchain != VK_NULL_HANDLE) {
        ...
        res = vkCreateSwapchainKHR(wsi->ctx->device, &wsi->sc_info, NULL,
                                   &wsi->swapchain);
        vkDestroySwapchainKHR(wsi->ctx->device, wsi->sc_info.oldSwapchain,
                              NULL);
    } else {
        res = vkCreateSwapchainKHR(wsi->ctx->device, &wsi->sc_info, NULL,
                                   &wsi->swapchain);
    ...
    VkImage *imgs;
    unsigned img_n;
    res = vkGetSwapchainImagesKHR(wsi->ctx->device, wsi->swapchain, &img_n,
                                  NULL);
    if (res != VK_SUCCESS) {
        yf_seterr(YF_ERR_DEVGEN, __func__);
        return -1;
    ...
    for (size_t i = 0; i < img_n; i++) {
        res = vkCreateSemaphore(wsi->ctx->device, &sem_info, NULL,
                                wsi->imgs_sem+i);
        if (res != VK_SUCCESS) {
...
YF_wsi yf_wsi_init(YF_context ctx, YF_window win)
{
    ...
    if (init_surface(wsi) != 0 || query_surface(wsi) != 0 ||
...
int yf_wsi_next(YF_wsi wsi, int nonblocking)
{
    ...
    case VK_SUCCESS:
        ...
        yf_cmdexec_waitfor(wsi->ctx, wsi->imgs_sem[img_i],
