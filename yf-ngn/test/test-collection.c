...
{
    struct { char *name; yf_node_t *node; } nodes[] = {
        {"node1", yf_node_init()},
        {"node2", yf_node_init()},
        {"node3", yf_node_init()},
        {"node4", yf_node_init()}
    ...
    struct { char *name; yf_material_t *matl; } matls[] = {
    ...
    for (size_t i = 0; i < (sizeof nodes / sizeof *nodes); i++) {
        ...
        if (yf_collec_manage(coll, YF_CITEM_NODE, nodes[i].name, nodes[i].node)
            != 0)
            return -1;

        snprintf(s, sizeof s, "coll, CITEM_NODE, \"%s\"", nodes[i].name);
        YF_TEST_PRINT("contains", s, "");
        if (!yf_collec_contains(coll, YF_CITEM_NODE, nodes[i].name))
            return -1;
    }
    ...
    for (size_t i = 0; i < (sizeof matls / sizeof *matls); i++) {
        ...
        YF_TEST_PRINT("manage", s, "");
        if (yf_collec_manage(coll, YF_CITEM_MATERIAL, matls[i].name,
                             matls[i].matl) != 0)
            return -1;

        snprintf(s, sizeof s, "coll, CITEM_MATERIAL, \"%s\"", matls[i].name);
        YF_TEST_PRINT("contains", s, "");
        if (!yf_collec_contains(coll, YF_CITEM_MATERIAL, matls[i].name))
            return -1;
    }
    ...
    if (yf_collec_getitem(coll, YF_CITEM_NODE, nodes[1].name) != nodes[1].node)
        return -1;

    snprintf(s, sizeof s, "coll, CITEM_NODE, \"%s\"", nodes[0].name);
    YF_TEST_PRINT("getitem", s, "");
    if (yf_collec_getitem(coll, YF_CITEM_NODE, nodes[0].name) != nodes[0].node)
        return -1;

    snprintf(s, sizeof s, "coll, CITEM_MATERIAL, \"%s\"", matls[1].name);
    YF_TEST_PRINT("getitem", s, "");
    if (yf_collec_getitem(coll, YF_CITEM_MATERIAL, matls[1].name)
        != matls[1].matl)
        return -1;

    snprintf(s, sizeof s, "coll, CITEM_MATERIAL, \"%s\"", matls[0].name);
    YF_TEST_PRINT("getitem", s, "");
    if (yf_collec_getitem(coll, YF_CITEM_MATERIAL, matls[0].name)
        != matls[0].matl)
        return -1;
    ...
    YF_TEST_PRINT("getitem", s, "");
    if (yf_collec_getitem(coll, YF_CITEM_NODE, matls[1].name) != NULL ||
    ...
    YF_TEST_PRINT("getitem", s, "");
    if (yf_collec_getitem(coll, YF_CITEM_MATERIAL, nodes[0].name) != NULL ||
        yf_geterr() != YF_ERR_NOTFND)
        return -1;
    yf_printerr();
    ...
    if (yf_collec_getitem(coll, YF_CITEM_SCENE, "scene1") != NULL ||
        yf_geterr() != YF_ERR_NOTFND)
        return -1;
    yf_printerr();
    ...
    for (size_t i = 0; i < (sizeof nodes / sizeof *nodes); i++) {
        ...
        if (yf_collec_release(coll, YF_CITEM_NODE, nodes[i].name)
            != nodes[i].node)
            return -1;
    ...
    for (size_t i = 0; i < (sizeof matls / sizeof *matls); i++) {
        snprintf(s, sizeof s, "coll, CITEM_MATERIAL, \"%s\"", matls[i].name);
        YF_TEST_PRINT("release", s, "");
        if (yf_collec_release(coll, YF_CITEM_MATERIAL, matls[i].name)
            != matls[i].matl)
            return -1;
    ...
    yf_collec_t *coll2 = yf_collec_init("tmp/scene.glb");
    if (coll2 == NULL)
        return -1;
