...
int yf_test_skin(void)
{
    ...
    yf_skin_t *skin = yf_skin_init(jnts, 5);
    if (skin == NULL)
        return -1;
    ...
    yf_skeleton_t *skel = yf_skin_makeskel(skin, NULL);
    if (skel == NULL)
        return -1;
    ...
    yf_skeleton_t *skel2 = yf_skin_makeskel(skin, NULL);
    if (skel2 == NULL)
        return -1;
    ...
    yf_skin_t *skin2 = yf_skin_init(jnts, 2);
    if (skin2 == NULL)
        return -1;

    yf_print_skin(skin2);

    yf_node_t *nodes[] = {node, node};
    ...
    yf_node_t *node2 = yf_skin_getnode(skin2, skel2);
    if (node2 == NULL)
        return -1;

    YF_TEST_PRINT("getjntnode", "skin2, skel2, 0", "");
    yf_node_t *node3 = yf_skin_getjntnode(skin2, skel2, 0);
    if (node3 == NULL || node3 == node2)
        return -1;

    YF_TEST_PRINT("getjntnode", "skin2, skel2, 1", "");
    yf_node_t *node4 = yf_skin_getjntnode(skin2, skel2, 1);
