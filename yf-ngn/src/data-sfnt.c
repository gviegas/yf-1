...
int yf_loadsfnt(const char *pathname, YF_fontdt *data)
{
  ...
  if (cmap_off == 0 || head_off == 0 || hhea_off == 0 ||
      hmtx_off == 0 || maxp_off == 0 || cmap_len < YF_SFNT_CMAPHSZ ||
      head_len != YF_SFNT_HEADSZ || hhea_len != YF_SFNT_HHEASZ ||
      hmtx_len < YF_SFNT_HMTXESZ || maxp_len != YF_SFNT_MAXPSZ) {
    yf_seterr(YF_ERR_INVFILE, __func__);
    deinit_tables(&sfnt);
    fclose(file);
    return -1;
...
static int load_ttf(T_sfnt *sfnt, FILE *file)
{
  ...
  if (gasp_off != 0 && gasp_len >= YF_SFNT_GASPHSZ) {
    ...
    if (rng_n > 0) {
      ...
      if (fread(sfnt->ttf.gasp->gaspes, sizeof(T_gaspe), rng_n, file) < rng_n) {
        yf_seterr(YF_ERR_INVFILE, __func__);
        return -1;
      }
    }
